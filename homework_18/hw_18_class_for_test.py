...
class PersonVerify:
    ...
    def verify_full_name(cls, full_name: str):
        if not isinstance(full_name, str):
            raise TypeError('Full name must be a string') #проверяем первое условие - имя не цифра

        full_name = full_name.split(' ')

        if not [value for value in full_name if value]:
            raise TypeError('Full name must contain at least one character') #проверяем второе условие - имя не пустое

        if len(full_name) != 3:
            raise TypeError('Invalid name format') #проверяем третье условие - имя не менее трех слов (если рассплитить)

        for name in full_name:
            if len(name.strip(cls.LETTERS)) != 0:
                raise TypeError('Full name can only contain letter') #проверяем четвертое условие - имя не содержит цифр
